...
BASE_COMMIT = "21033795c0a7c175611ab6ff6c9bab48728bae4a"
...
RESULT_ROOT = ROOT / "tfpd_exploration/h1_series_20260830/results/h1_masked_dense_aux_v2_quiet_retry"
LOG_DIR = ROOT / "logs/h1_masked_dense_aux_v2_quiet_retry"
...
PRIOR_V2_RESULT = ROOT / "tfpd_exploration/h1_series_20260830/results/h1_masked_dense_aux_v2_final_legal_subset"
...
LOGGING_AMENDMENT = ROOT / "tfpd_exploration/h1_series_20260830/docs/AMENDMENT_H1_MASKED_DENSE_AUX_V2_QUIET_PROGRESS_20260831.md"
CLOSURE = (
...
ORIGINAL_TRAINER = engine.pl.Trainer


def quiet_trainer(*args, **kwargs):
    """Suppress non-semantic tqdm output in per-cell redirected logs."""
...
def configure_engine() -> None:
    ...
    engine.pl.Trainer = quiet_trainer
...
def preflight(log_path: Path) -> None:
    ...
    prior_failure_path = PRIOR_V2_RESULT / "failure.json"
    prior_failure_sha = verify_immutable(prior_failure_path)
    prior_failure = engine.load_json(prior_failure_path)
    if (prior_failure.get("status") != "FAIL_H1_MASKED_DENSE_AUX_V2_NO_RETRY"
            or prior_failure.get("target_opened") is not False
            or prior_failure.get("formal_heldout_opened") is not False):
        raise RuntimeError("prior V2 failure is not eligible for the user-authorized quiet retry")
    ...
    write_immutable_json(RESULT_ROOT / "attempt.json", {
        ...
        "prior_v2_failure_path": str(prior_failure_path),
        "prior_v2_failure_sha256": prior_failure_sha,
        "retry_authority": "user selected option 2: terminate and restart from epoch 0",
        "progress_bar": False,
