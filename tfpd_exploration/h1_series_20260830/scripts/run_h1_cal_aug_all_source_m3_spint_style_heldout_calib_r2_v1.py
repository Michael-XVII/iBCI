#!/usr/bin/env python3
"""Run the additive local original-SPINT-style H1 held-out-calib diagnostic."""
...
from src.h1_cal_aug_all_source_m3_spint_style_heldout_calib_r2_v1 import (  # noqa: E402
    SCHEMA,
    create_attempt,
    evaluate,
    load_attempt,
...
DEFAULT_PREDECESSOR_ROOT = REPO_ROOT / "tfpd_exploration/h1_series_20260830/results/h1_cal_aug_all_source_m3_deployment_v1_package_a1"
DEFAULT_RESULT_ROOT = REPO_ROOT / "tfpd_exploration/h1_series_20260830/results/h1_cal_aug_all_source_m3_spint_style_heldout_calib_r2_v1"
DEFAULT_LOG_ROOT = REPO_ROOT / "logs/h1_cal_aug_all_source_m3_spint_style_heldout_calib_r2_v1"
WORK_ORDER = REPO_ROOT / "tfpd_exploration/h1_series_20260830/docs/WORKORDER_H1_CAL_AUG_ALL_SOURCE_M3_SPINT_STYLE_HELDOUT_CALIB_R2_V1.md"
TEST_FILE = SPINT_ROOT / "tests/test_h1_cal_aug_all_source_m3_spint_style_heldout_calib_r2_v1.py"
MODULE = SPINT_ROOT / "src/h1_cal_aug_all_source_m3_spint_style_heldout_calib_r2_v1.py"


def closure() -> dict[str, str]:
    paths = (
        WORK_ORDER, Path(__file__).resolve(), TEST_FILE, MODULE,
    ...
    if missing:
        raise RuntimeError(f"diagnostic closure incomplete: {missing}")
...
def cpu_gate(result_root: Path) -> None:
    ...
    publish_json(result_root.resolve() / "cpu_gate.json", {
        ...
        "status": "PASS_H1_M3_SPINT_STYLE_HELDOUT_CALIB_CPU_GATE" if completed.returncode == 0 else "FAIL_H1_M3_SPINT_STYLE_HELDOUT_CALIB_CPU_GATE",
    ...
    if completed.returncode:
        raise RuntimeError("diagnostic CPU gate failed")
...
def publish_failure(result_root: Path, phase: str, error: BaseException) -> None:
    ...
    publish_json(path, {
        "schema": SCHEMA,
        "status": "FAIL_H1_M3_SPINT_STYLE_HELDOUT_CALIB_NO_AUTOMATIC_RETRY",
        "phase": phase,
...
def dry_plan() -> dict:
    return {
        ...
        "training": False,
        "evalai_submissions": 0,
        "phases": ["cpu-gate", "attempt", "predecessor-authority", "held-out-calib-evaluation", "terminal-recomputation"],
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.initialize:
            create_attempt(args.result_root, args.predecessor_root, closure(), git_head())
        ...
        validate_predecessor(args.predecessor_root, args.result_root)
        phase = "heldout_calib_evaluation"
        evaluate(args.data_root, args.predecessor_root, args.result_root, device=args.device)
        phase = "terminal_recomputation"
        verify_terminal(args.result_root)
        return 0
